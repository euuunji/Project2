...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet var loginHelp: UILabel!
   ...
   func textFieldShouldReturn(_ textField: UITextField) -> Bool {
    ...
    @IBAction func buttonLogin() {
        if loginID.text == "" {
            loginHelp.text = "ID를 입력하세요";
            return;
        }
        if loginPW.text == "" {
            loginHelp.text = "비밀번호를 입력하세요";
            return;
        }
        
        let urlString: String = "http://condi.swu.ac.kr/student/M05/login/loginUser.php"
        ...
        let task = session.dataTask(with: request) { (responseData, response, responseError) in
            ...
            do {
                ...
                if let name = jsonData["name"] as? String { DispatchQueue.main.async {
                    self.loginHelp.text = "Hello~ " + name
                    ...
                    DispatchQueue.main.async { self.loginHelp.text = errMessage }
                    }
                }
